...
public:
  ...
  SimpleHttp(int serverport, int connections)
      : port(serverport), max_connections(connections) {}
  ...
  bool init() {
    ...
    accepting_thread =
        std::thread(&SimpleHttp::startListening, this, listening_socket);
  ...
  void registerFunc(const std::string &endpoint, HttpHandleFunc func) {
    func_map[endpoint] = func;
  }

  static bool sendResponse(const int client, const std::string &message,
                           int response_number,
